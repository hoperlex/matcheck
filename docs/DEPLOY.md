...
- [Воркер УПД](#воркер-упд)
- [Приём УПД из почты](#приём-упд-из-почты)
- [NGINX](#nginx)
...
- `matcheck-mail-worker` — опрос почтовых ящиков (см. [Приём УПД из почты](#приём-упд-из-почты)). Тоже образ API, команда `src/mail-worker.ts`. Поднимается отдельно и без ящиков с включённым опросом ничего не делает; `deploy.sh` обновляет его, только если контейнер уже создан.
...
Команда без списка сервисов поднимает и `matcheck-mail-worker`. Это безопасно:
без `MAIL_POLL_ENABLED=true` и ящика с включённым опросом он ничего не
опрашивает. Чтобы поднять стек без него, перечислите сервисы явно:
`up -d matcheck-redis matcheck-api matcheck-web matcheck-worker`.
...
Подрядчики присылают УПД на выделенный ящик; письмо с явно указанным объектом
становится документом само, без участия менеджера. Всё остальное попадает на
вкладку «Разбор почты» в разделе «Документы».

- **Процесс**: контейнер `matcheck-mail-worker` — тот же образ, что у API,
  команда `tsx src/mail-worker.ts`. Отдельный от `matcheck-worker`, потому что
  тот занят распознаванием PDF по несколько минут и IMAP-сессия встала бы в ту
  же очередь.
- **Две независимые защиты от случайного включения**: `MAIL_POLL_ENABLED` в
  `api.env` и `poll_enabled` у конкретного ящика в админке. Без обоих процесс
  живёт, но сам не опрашивает ничего.
- **Чтение неразрушающее**: письма забираются через `BODY.PEEK`, поэтому флаг
  «прочитано» не ставится и ничего не удаляется — ящик можно параллельно
  смотреть обычной почтой.
- `scripts/deploy.sh` подхватывает контейнер автоматически, если тот уже
  создан. Пока почта не запущена, деплой о нём не знает.

### Порядок запуска

```bash
# [matcheck@mosgate:/srv/matcheck/app]$
# 1. Переменные MAIL_* в /srv/matcheck/secrets/api.env (см. infra/api.env.example).
#    MAIL_POLL_ENABLED пока оставить false.

# 2. Поднять контейнер — он будет обслуживать только ручную проверку.
docker compose -f infra/docker-compose.prod.yml up -d matcheck-mail-worker
docker compose -f infra/docker-compose.prod.yml logs --tail=50 matcheck-mail-worker
```

Дальше — в интерфейсе, под администратором:

1. **Админ → Почта** → добавить ящик: IMAP-хост, порт 993, TLS, логин и пароль.
   Для Яндекс 360 при двухфакторной аутентификации нужен **пароль приложения**,
   обычный не подойдёт. Назначение — «Документы (УПД)», автоопрос выключен.
2. Нажать **«Проверить сейчас»** — так проверяются доступы, не включая
   постоянный опрос. Результат смотреть в логах контейнера и на вкладке
   «Разбор почты».
3. Убедиться, что письма забираются, и только после этого включить автоопрос
   у ящика.
4. Разослать подрядчикам формат: строка `Объект: <КОД>` в теме или первой
   строке письма. Без неё письмо не потеряется, но попадёт в разбор и будет
   ждать оператора.
...
```bash
# [matcheck@mosgate:/srv/matcheck/app]$
docker compose -f infra/docker-compose.prod.yml logs -f --tail=100 matcheck-mail-worker
```

Письма, которые не удалось забрать (больше лимита, обрыв связи, битый MIME),
видны на вкладке «Разбор почты» отдельным блоком с кнопкой «Забрать снова» —
она ставит точечный дозабор, не сдвигая границу ящика и не перечитывая
соседние письма.

Письма и вложения лежат в объектном хранилище (S3), на диске сервера почта не
хранится вовсе — только логи контейнера с ротацией (5 файлов по 20 МБ).

По умолчанию письма **не удаляются**: `MAIL_RETENTION_DAYS=0`. Оригинал письма —
доказательство «подрядчик прислал вот это и вот тогда», поэтому автоудаление
включается осознанно. Порядок объёма: при двадцати письмах в день — примерно
2–3 ГБ в год.

Если задать срок (например, `365`), удаляются только сырые письма и временные
копии вложений — у писем, с которыми уже разобрались. Документы, их постоянные
файлы, приёмки и фотографии не затрагиваются: при создании пакета вложение
копируется в постоянный ключ документа. Письма, ожидающие разбора, не
удаляются никогда.
...
### Требования vhost для публичной загрузки документов

Страница `/uploads` открыта без авторизации: поставщик сам грузит документы приёмки
(`/api/v1/public/*`). От vhost зависят две вещи, которые снаружи не видно, пока не станет поздно:

- **`proxy_set_header X-Real-IP $remote_addr;`** — по этому заголовку считается ограничение
  «5 отправок с адреса». `X-Forwarded-For` для этого не годится: nginx его не заменяет, а дополняет
  (`$proxy_add_x_forwarded_for`), поэтому клиент подделает себе новый лимит каждым запросом. Без
  `X-Real-IP` защита от спама фактически отключена.
- **`client_max_body_size` не меньше `25m`** — иначе крупные пачки фото режет сам nginx, отвечая
  HTML-страницей мимо API, и поставщик видит невнятную ошибку вместо «слишком большой объём».

### Потолки приёма в день массовой загрузки

Оба лимита живут в `api.env` и считают **запросы**, не файлы:

| Переменная | Умолчание | Смысл |
|---|---|---|
| `PUBLIC_UPLOAD_GLOBAL_MAX` | `200` | запросов в час на всю форму |
| `PUBLIC_UPLOAD_IP_MAX` | `20` | запросов за 10 минут с одного адреса |

Умолчания подобраны не «с запасом», а по мощности разбора: воркер при `CONCURRENCY=1`
переваривает 200–300 файлов в час, а на один запрос по боевым манифестам приходится ~1,1 файла.
Поднять потолок, не подняв мощность разбора, значит копить очередь: поставщик отправит документы,
а статус будет ждать часами.

Перед тем как что-то менять, смотреть факты в логах API — строка `public upload: принято`,
поле `updParseWaiting` (глубина очереди `upd-parse` на момент приёма):

```bash
# [matcheck@mosgate:/srv/matcheck/app]$
docker compose -f infra/docker-compose.prod.yml logs --tail=2000 matcheck-api \
  | grep 'public upload: принято'
```

Очередь от отправки к отправке растёт — упёрлись в разбор, поднимать лимит бессмысленно.
Очередь около нуля, а поставщики ловят 429 — потолок можно поднять. Применяется как обычная
правка `api.env` (см. «При обновлении только `api.env`»), выката образа не требует.

### Аварийно закрыть публичный приём файлов

Если с публичной страницы пошёл поток мусора, приём выключается на nginx без выката:

```nginx
# в vhost, рядом с location /api/v1/
location = /api/v1/public/upload-documents {
...
Закрывается **только приём файлов**. Справочник объектов и проверка статуса уже принятых обращений
продолжают работать — поставщик с номером на руках не упирается в отказ по чужой вине. Обратно —
убрать блок и снова `nginx -t && systemctl reload nginx`.

Постоянная защита работает всегда и от этой процедуры не зависит: лимиты по IP и общий потолок в час,
скрытое поле-ловушка для ботов, определение типа файла по содержимому, ограничения объёма.
