...
и `429` — оба кода сняты, обрабатывать их клиенту больше не нужно.

Единственное следствие серии неверных попыток — растущая пауза перед ответом
(до 30 секунд), поэтому таймаут запроса логина стоит держать не ниже 45 секунд.
Серия обнуляется успешным входом либо часом без единой ошибки.

### POST `/api/v1/auth/refresh`

Принимает refresh-token в заголовке `Authorization: Bearer` (для mobile) либо из cookie `__Host-refresh` (для web). Ротация: старый refresh-token инвалидируется при первом успешном использовании.

**Повтор уже использованного refresh.** Первые `REFRESH_REUSE_GRACE_SECONDS` (по умолчанию 60) после ротации сервер считает повтор потерянным ответом, а не кражей: возвращает 200 и **ту же самую замену**, что выдал в первый раз (новой ротации при этом не происходит, срок жизни токена не продлевается). Это спасает клиента, до которого ответ не дошёл — оборвался по таймауту, потерялся при обрыве связи, ушёл вторым параллельным запросом.

Повтор перестаёт быть безопасным и даёт **401 + полную инвалидацию всей сессии** (прежняя защита от кражи), если: окно истекло; выданная замена сама уже ротирована дальше, то есть клиент отстал на два шага; замена отозвана, просрочена или принадлежит другой сессии.

Отсюда важное для клиента: **свой mutex вокруг refresh снимать нельзя**. Серверное окно — страховка на случай потерянного ответа, а не разрешение слать refresh параллельно: отстав на два шага, клиент по-прежнему получит разлогин.
...
- `401 invalid_refresh` / `401 no_refresh` — сессия мертва: чистим токены, уходим на экран входа
- `429 rate_limit_exceeded` — чаще 60 обновлений в минуту с одного адреса

`429` **не означает смерть сессии**: refresh-token остаётся действительным, токены
сбрасывать нельзя и разлогинивать пользователя тоже. Правильная реакция — подождать
и повторить (в `message` указано, через сколько секунд). Разлогин допустим строго
на `401` — так и сделано в `TokenAuthenticator` Android-клиента.

До версии, где этот код появился, превышение лимита возвращало `500` —
клиенты, ориентировавшиеся на «5xx → backoff», сохранят прежнее поведение.
