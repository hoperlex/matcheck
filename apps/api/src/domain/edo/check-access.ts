...
export function describeClientProbe(probe: ClientAuthProbe): string {
  if (probe.outcome === 'client_accepted') {
    if (probe.method === 'basic') {
      // Диадок требует передавать реквизиты в теле, поэтому такой ответ означает
      // расхождение между регистрацией приложения и документацией. Штатный путь
      // портала сознательно остаётся одним — сообщаем факт, а не обещаем починку.
      return (
        ' Дополнительная проверка: ключи принимаются только при передаче заголовком Authorization,' +
        ' хотя документация Диадока требует передавать их в теле запроса. Это расхождение в' +
        ' регистрации приложения — покажите этот текст поддержке Контура вместе с идентификатором' +
        ' запроса.'
      );
    }
    return (
  ...
  if (probe.outcome === 'client_rejected') {
    return (
      ...
      ' сервис не доходит до проверки refresh-токена — отвергается сама пара client_id и ключ' +
      ' приложения. Остаются две причины. Первая: в поле ключа не то значение — сверьте его с' +
      ' «Ключом API» того же приложения, что и client_id (это и есть client_secret). Вторая:' +
      ' регистрация приложения на стороне Контура — ключ перевыпущен или отвязан, либо изменения' +
      ' ещё не применились. Если ключ совпадает, обратитесь в поддержку с идентификатором запроса' +
      ' и точным временем.'
    );
  }
