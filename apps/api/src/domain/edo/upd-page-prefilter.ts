...
export type PageType =
  ...
  | 'upd_continuation'
  | 'transport_waybill'
  // Форма М-15 (отпуск материалов на сторону). Тип возвращает только промпт
  // PAGE_CLASSIFY_TORG12_PROMPT: прежний текст о ней не спрашивает и вернуть
  // её не может, поэтому добавление значения не меняет разбор старых ответов.
  //
  // В отбор страниц он НЕ входит: 'm15' ведёт себя ровно как 'other' —
  // страница остаётся в сегменте. Отбрасывать её нельзя, пока у сборки нет
  // маршрута для файла, все страницы которого отброшены: такой файл остался
  // бы вообще без документа (сейчас по отброшенным страницам ставится только
  // пометка dropped_pages_not_parsed). Значение нужно для наблюдаемости —
  // в улике видно, что модель распознала именно М-15, а не «прочее»,
  // и по этому же признаку откат ведёт файл к своему парсеру.
  | 'm15'
  | 'certificate'
  | 'other';
...
export type PrefilterOpts = {
  ...
  /**
   * Текст промпта классификации. Пусто — PAGE_CLASSIFY_PROMPT, то есть
   * прежнее поведение побайтово. Заполняется вызывающим, когда включён
   * рубильник PAGE_CLASSIFY_TORG12: сам модуль окружение не читает.
   */
 ...
 */
const DOC_NUMBER_SUFFIX = `Дополнительно для каждой страницы верни поле "docNumber":
...
/**
 * Тот же классификатор, но знающий товарную накладную ТОРГ-12 и форму М-15.
 *
 * Зачем отдельным текстом. Прежний промпт описывает УПД через «Счёт-фактура №»
 * и графы 1137, а ТОРГ-12 не знает вовсе — и уверенно называет её шапку
 * transport_waybill: в правом верхнем углу бланка напечатана рамка
 * «Транспортная накладная» с номером и датой. Страница отбрасывается как
 * чужая, пакет откатывается с «нет ни одной УПД-страницы», а её позиции
 * теряются (58 пакетов за 30 дней, боевой пример — накладная 1002004449 от
 * 21.09.2026).
 *
 * Тип "m15" добавлен здесь же и не ради классификации ради классификации:
 * страница настоящей М-15, попавшая в собираемый пакет, обязана быть
 * отброшена. Без собственного типа она получает 'other', а 'other' остаётся
 * в сегментах намеренно (страница не должна теряться) — и позиции М-15
 * влились бы в чужую УПД.
 *
 * Прежний текст не трогаем: он остаётся тем же побайтово и продолжает
 * работать, пока PAGE_CLASSIFY_TORG12 выключен.
 */
export const PAGE_CLASSIFY_TORG12_PROMPT = `Ты классифицируешь страницы отсканированного пакета документов (поставка материалов).
Тебе переданы изображения страниц по порядку, перед каждым указан её номер ("Страница N:").
...
Допустимые значения type:
- "upd_main" — основная страница товарного документа: реквизиты сторон плюс начало таблицы позиций. Это либо УПД / счёт-фактура по форме ПП №1137 (заголовок «Счёт-фактура №», графы 1, 1а, 2..11), либо ТОВАРНАЯ НАКЛАДНАЯ по форме ТОРГ-12 (код по ОКУД 0330212, заголовок «ТОВАРНАЯ НАКЛАДНАЯ», поля «Грузоотправитель», «Грузополучатель», «Поставщик», «Плательщик», «Номер документа», «Дата составления», таблица граф 1..15 с «Сумма с учётом НДС»).
- "upd_continuation" — продолжение табличной части ТОГО ЖЕ документа на следующей странице: шапки нет, но видна продолжающаяся таблица позиций. У ТОРГ-12 это лист с графами 1..15, строками «Итого» и «Всего по накладной», строками «Масса груза (нетто)», «Масса груза (брутто)» и подписями «Отпуск груза произвёл», «Груз получил».
- "transport_waybill" — транспортная/товарно-транспортная накладная (ТН, ТТН, форма Минтранса, разделы «Грузоотправитель», «Перевозчик», «Приём груза», «Сдача груза»). ВАЖНО: рамка «Транспортная накладная» с номером и датой в правом верхнем углу бланка ТОВАРНОЙ накладной ТОРГ-12 — это ссылка на сопровождающий документ, страницей транспортной накладной она НЕ является. Коды «Форма по ОКУД», «по ОКПО» в углу бланка признаком ТН тоже не являются.
- "m15" — накладная на отпуск материалов на сторону (типовая межотраслевая форма М-15) или требование-накладная: заголовок «на отпуск материалов на сторону», графы «Корреспондирующий счёт» и «Через кого», подписи «Затребовал», «Разрешил».
- "certificate" — сертификат или паспорт качества/количества, декларация соответствия.
- "other" — спецификация, акт, доверенность, рукописные/прочие листы.
...
- Нумеруй страницы с 1 строго в порядке переданных изображений, не пропускай ни одной.
- Страница повёрнута боком? Всё равно классифицируй по содержимому.
- Если сомневаешься между товарным документом (upd_main/upd_continuation) и чужим — выбирай чужой только когда явно видишь признаки транспортной накладной, формы М-15 или сертификата; иначе считай страницу частью товарного документа.
- Верни ровно один JSON-объект, без markdown-ограждений и пояснений.`;

/**
 * Тот же промпт с ТОРГ-12 плюс номер документа со страницы.
 *
 * Суффикс общий с прежней версией — к нему добавлена одна строка про графу
 * «Номер документа» бланка ТОРГ-12: без неё модель берёт номер из рамки
 * транспортной накладной, напечатанной рядом (боевой случай — 10426043772
 * вместо 1002004449).
 */
export const PAGE_CLASSIFY_TORG12_WITH_NUMBER_PROMPT = `${PAGE_CLASSIFY_TORG12_PROMPT}

${DOC_NUMBER_SUFFIX}

Для товарной накладной ТОРГ-12 номером документа является значение графы «Номер документа» в таблице рядом с «Дата составления». Номер транспортной накладной из рамки в правом верхнем углу, коды по ОКУД и по ОКПО номером документа не являются.`;

/**
 * Выбор текста промпта по двум независимым признакам.
 *
 * Чистая функция без чтения окружения: рубильники живут у вызывающих (сборка
 * в worker, одиночный путь в parseUpdVision), а здесь остаётся только
 * соответствие «набор признаков → текст». Так все четыре сочетания
 * рубильников проверяются тестом, а не собираются на месте вызова.
 */
export function pageClassifyPrompt(opts: {
  /** UPD_ASSEMBLY_SPLIT_BY_DOC_NUMBER !== 'off' */
  withDocNumber?: boolean;
  /** PAGE_CLASSIFY_TORG12 */
  torg12?: boolean;
}): string {
  if (opts.torg12) {
    return opts.withDocNumber ? PAGE_CLASSIFY_TORG12_WITH_NUMBER_PROMPT : PAGE_CLASSIFY_TORG12_PROMPT;
  }
  return opts.withDocNumber ? PAGE_CLASSIFY_WITH_NUMBER_PROMPT : PAGE_CLASSIFY_PROMPT;
}
  ...
  if (totalPages <= 1) {
    ...
    try {
      const c = await classifyPages({
        ...
        model: opts.model,
        thumbs,
        ...(opts.classifyPrompt ? { prompt: opts.classifyPrompt } : {}),
      });
