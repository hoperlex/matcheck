...
const RESPONSE_JSON_SCHEMA = {
  ...
  properties: {
    ...
    // Сырой текст той же графы 4, как напечатан. Структурированный consignee —
    // суждение модели, а сырая строка — свидетельство: по ней отличают
    // буквальное «он же» от молчаливо скопированного покупателя. Спрашивает
    // промпт v14; на старых версиях приходит null, и это штатно.
    consigneeRaw: { type: ['string', 'null'] },
    items: {
      type: 'array',
      items: {
        type: 'object',
        ...
        properties: {
          ...
          rowNo: {
            type: ['integer', 'null'],
            description:
              'Порядковый номер позиции из графы 1 бланка, как напечатан. Нужен для сверки целостности списка: пропущенная или задвоенная строка видна даже когда суммы сошлись.',
          },
          ...
          vatRate: {
            type: ['number', 'null'],
            description:
              'Налоговая ставка по строке в процентах: 22, 20, 10, 0. ' +
              'С 2026 года основная ставка — 22, значение 20 встречается только в документах ' +
              'прошлых лет. null — если в строке «Без НДС» или прочерк.',
          },
          vatSum: {
            type: ['number', 'null'],
            description:
