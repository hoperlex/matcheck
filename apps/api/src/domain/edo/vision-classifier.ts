...
export const CLASSIFY_PROMPT = `Определи ТИП документа на изображении и ответь СТРОГО одним JSON-объектом: {"kind": "<тип>", "confidence": <0..1>}.
...
/**
 * Тот же классификатор, знающий товарную накладную ТОРГ-12.
 *
 * Зачем. Прежний текст описывает УПД через «Счёт-фактура №» и графы 1–11, а
 * ТОРГ-12 не упоминает вовсе — и лист такой накладной с таблицей позиций
 * модель уверенно называет формой М-15 (боевой случай: «лист 2.jpg», kind
 * m15, confidence 0.95). Файл уходит отдельным документом ещё до пакетной
 * классификации и уже не может склеиться со своей шапкой, а его количество и
 * цена читаются по чужим графам: в цену попадает масса нетто.
 *
 * Новых значений kind не вводим: ТОРГ-12 → 'upd'. Ровно так же поступает
 * текстовый классификатор (document-router, сигнал 'text:torg12') — для
 * файла с текстовым слоем это правило работает давно, здесь оно
 * распространяется на фото, где текста нет.
 *
 * Прежний текст остаётся тем же побайтово и работает, пока рубильник
 * ROUTER_CLASSIFY_TORG12 выключен.
 */
export const CLASSIFY_PROMPT_TORG12 = `Определи ТИП документа на изображении и ответь СТРОГО одним JSON-объектом: {"kind": "<тип>", "confidence": <0..1>}.

Возможные значения kind:
- "upd" — товарный документ поставки: счёт-фактура / универсальный передаточный документ (УПД) по форме ПП №1137 (заголовок «Счёт-фактура №…», графы 1–11) ИЛИ ТОВАРНАЯ НАКЛАДНАЯ по форме ТОРГ-12 (код по ОКУД 0330212, заголовок «ТОВАРНАЯ НАКЛАДНАЯ», поля «Грузоотправитель», «Грузополучатель», «Поставщик», «Плательщик», «Номер документа», «Дата составления», таблица граф 1–15 с «Сумма с учётом НДС»). Сюда же относится ЛИСТ-ПРОДОЛЖЕНИЕ такого документа: шапки нет, но видна его таблица позиций со строками «Итого», «Всего по накладной», «Масса груза (нетто)» и подписями «Отпуск груза произвёл», «Груз получил».
- "transport_waybill" — транспортная накладная (ТН, форма 2116) ИЛИ накладная на внутреннее перемещение основных средств (форма ОС-2). Рамка «Транспортная накладная» с номером и датой в правом верхнем углу бланка ТОВАРНОЙ накладной ТОРГ-12 — это ссылка на сопровождающий документ, транспортной накладной её не делает.
...
export async function classifyImageKind(
  ...
  try {
    ...
    // Рубильник читается здесь, а не у вызывающего: классификатор зовут из
    // router и из фото-пути, и оба обязаны спрашивать модель одинаково.
    // Выключен — текст ровно прежний.
    const promptText = loadEnv().ROUTER_CLASSIFY_TORG12 ? CLASSIFY_PROMPT_TORG12 : CLASSIFY_PROMPT;
