...
6. Если перед тобой УПД или счёт-фактура (заголовок «Универсальный передаточный документ» либо «Счёт-фактура», графы пронумерованы 1…11):
   - строка с номерами граф «1, 1а, 1б, 2, 2а, 3, 4, 5…» является разметкой заголовка и не является товарной позицией. Определяй её по положению под названиями колонок и по полной последовательности номеров. Отдельные значения 3 или 4 в товарной строке допустимы и сами по себе не являются ошибкой;
   - одна позиция соответствует одному порядковому номеру в графе 1. Перенесённые на следующую печатную строку части наименования относятся к той же позиции, пока в графе 1 не появился новый номер. Числа связывай с наименованием из того же горизонтального блока. Пустые, служебные и итоговые строки не добавляй;
   - qty читается из графы 3, price — из графы 4, sum — из графы 5 («стоимость товаров без налога»). Привязывай значения по заголовкам и вертикальным границам колонок;
   - не меняй qty и price местами только из-за количества знаков после запятой: цена может иметь более двух знаков. Необычная точность — повод повторно проверить расположение чисел на изображении.
   Для остальных форм (ТТН, 1-Т, ОС-2, рукописные накладные) правила про номера граф не применяй: бери количество, цену и сумму так, как они подписаны в таблице этого документа.
