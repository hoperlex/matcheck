...
/**
 * Ключ АВТОПОВТОРА сегмента (пересборка того же сегмента после расхождения).
 *
 * Отдельный и от segmentDispatchKeyOf, и от ключа второго прохода. От первого —
 * потому что первичное задание сегмента живёт под ключом поколения СБОРКИ, и
 * повтор с тем же ключом BullMQ отбросил бы как дубль уже отработавшего. От
 * второго — потому что это другой путь распознавания: страницы манифеста, а не
 * файл документа, и восстановление обязано ставить его тем же ключом, иначе
 * сегмент распознается дважды.
 */
export function segmentRepairKeyOf(segmentId: string, generation = 0): string {
  return assertValidKey(['segment', segmentId, 'repair', generation].join(KEY_SEP));
}
