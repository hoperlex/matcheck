...
function classifyStrict(
  ...
  // HEIC принимается: раньше здесь стоял честный отказ, потому что vision
  // работает только с jpeg/png/webp/pdf и распознать такой файл было нечем.
  // Теперь он декодируется системным heif-convert перед отправкой модели
