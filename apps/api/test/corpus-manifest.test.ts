...
describe('corpus-manifest — покрытие сверки промптов', () => {
  ...
  it('количество × цена сходится со стоимостью строки', () => {
    // Проверка, которой не хватило с первого раза. Сверка суммы позиций с
    // итогом документа охраняет только СУММЫ: если срез колонки приклеил к
    // количеству цифру соседней графы (22 → 221) или к цене хвост количества
    // (40,27 → 540,27), сумма строки остаётся прежней и итог сходится. Так в
    // эталон уехали 7 позиций из 70, и первый же A/B объявил регрессом верный
    // ответ модели — то есть плохой эталон опаснее отсутствующего.
    //
    // База берётся как «графа 9 минус графа 8»: считать по графе 5 нельзя, её
    // читает тот же срез и портит тем же способом.
    for (const e of entries) {
      for (const doc of e.expectedDocuments ?? []) {
        for (const item of doc.items ?? []) {
          ...
          // Допуск рублёвый: цена печатается округлённой до копеек, а сумма
          // считается по неокруглённой — на больших количествах расхождение в
          // рубли законно. Ошибка среза сдвигает число в разы.
          const tolerance = Math.max(1, Math.abs(base) * 0.001);
          expect(
            Math.abs(qty * price - base),
            `${e.filename} · ${doc.docNumber} · строка ${item.rowNo}: ${qty} × ${price} против ${base.toFixed(2)}`,
