...
describe('проба аутентификации приложения', () => {
  ...
  it('отвергнутые ключи снимают подозрение с токена', () => {
    ...
    expect(text).toMatch(/не доходит до проверки refresh-токена/i);
    expect(text).toMatch(/Ключом API/i);
    ...
    // Названы ОБА оставшихся класса причин. Отказ по Basic ожидаем (Диадок
    // требует тело), поэтому исключать способ передачи им нельзя.
    expect(text).toMatch(/регистрация приложения/i);
    expect(text).not.toMatch(/способ передачи ни при чём/i);
  });
  ...
  it('принятые ключи при передаче заголовком называют способ, а не ключи', () => {
    ...
    // Портал этот способ не использует, поэтому текст сообщает о расхождении с
    // документацией, а не обещает, что «сейчас само заработает».
    expect(text).toMatch(/поддержке Контура/i);
    expect(text).not.toMatch(/портал теперь пробует/i);
