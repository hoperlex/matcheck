...
describe('validateUpdTotals — цена указана с НДС вместо графы 4', () => {
  /**
   * Класс, найденный на боевых данных: за 30 дней 155 строк в 36 документах.
   * Количество, умноженное на цену, даёт стоимость С НАЛОГОМ вместо стоимости
   * без него — модель прочитала колонку правее нужной, и строка завышена ровно
   * на ставку.
   *
   * Обычные денежные проверки его не видят: итог документа собран из тех же
   * завышенных строк и сходится сам с собой.
   */
  const opts = { detectRecognitionWarnings: true };

  it('ловит боевой случай УПД № 223379 (все строки завышены на 20 %)', () => {
    // Строка 1 документа как есть: 10 x 530.19 = 5301.90 — это графа 9,
    // тогда как стоимость без налога 4418.25. Налог выделен верно (20 %).
    const r = validateUpdTotals(
      {
        totalSum: 5301.9,
        vatSum: 883.65,
        items: [{ qty: 10, price: 530.19, sum: 5301.9, vatRate: 20, vatSum: 883.65 }],
      },
      opts,
    );
    expect(r.warnings ?? []).toContainEqual({ name: 'price_includes_vat', scope: { row: 1 } });
  });

  it('молчит на верно прочитанной строке', () => {
    // Скан УПД № АА0907-0004, размечен вручную по бланку: 12 x 908.20 = 10898.40
    // против стоимости без налога 10898.36 — сходится, цена правильная.
    const r = validateUpdTotals(
      {
        totalSum: 13296,
        vatSum: 2397.64,
        items: [{ qty: 12, price: 908.2, sum: 13296, vatRate: 22, vatSum: 2397.64 }],
      },
      opts,
    );
    expect((r.warnings ?? []).map((w) => w.name)).not.toContain('price_includes_vat');
  });

  it('молчит, когда количество равно единице и цена законно равна стоимости', () => {
    // Транспортные услуги из скана АА0907-0004: qty = 1, цена 4098.36 совпадает
    // со стоимостью БЕЗ налога. Таких позиций в корпусе 17 — принять их за
    // ошибку значило бы завалить очередь менеджера законными документами.
    const r = validateUpdTotals(
      {
        totalSum: 5000,
        vatSum: 901.64,
        items: [{ qty: 1, price: 4098.36, sum: 5000, vatRate: 22, vatSum: 901.64 }],
      },
      opts,
    );
    expect((r.warnings ?? []).map((w) => w.name)).not.toContain('price_includes_vat');
  ...
  it('НЕ путается с другим дефектом — неверно посчитанным НДС', () => {
    // Боевой документ 00-00078202: vatSum взят как 22 % ОТ СУММЫ С НАЛОГОМ,
    // а не выделен из неё. Фактическая ставка от базы — 28,2 % против
    // заявленных 22. Это другая болезнь, и лечится она иначе, поэтому здесь
    // признак предъявлять нельзя.
    const r = validateUpdTotals(
      {
        totalSum: 2451.15,
        vatSum: 539.25,
        items: [{ qty: 1, price: 2451.15, sum: 2451.15, vatRate: 22, vatSum: 539.25 }],
      },
      opts,
    );
    expect((r.warnings ?? []).map((w) => w.name)).not.toContain('price_includes_vat');
  ...
  it('молчит при нулевом налоге — там графы 5 и 9 совпадают законно', () => {
    const r = validateUpdTotals(
      {
        totalSum: 1000,
        vatSum: 0,
        items: [{ qty: 4, price: 250, sum: 1000, vatRate: 0, vatSum: 0 }],
      },
      opts,
    );
    expect((r.warnings ?? []).map((w) => w.name)).not.toContain('price_includes_vat');
  ...
  it('не срабатывает без явного запроса на предупреждения', () => {
    // Тот же документ № 223379, но detectRecognitionWarnings не передан:
    // старые вызовы валидации не должны внезапно получить новый ярлык.
    const r = validateUpdTotals({
      totalSum: 5301.9,
      vatSum: 883.65,
      items: [{ qty: 10, price: 530.19, sum: 5301.9, vatRate: 20, vatSum: 883.65 }],
  ...
  it('находит все девять строк боевого документа', () => {
    // Полный № 223379: признак должен срабатывать построчно, а не один раз на
    // документ — менеджеру нужно видеть, какие именно строки завышены.
    const rows: Array<[number, number, number, number]> = [
      [10, 530.19, 5301.9, 883.65],
      [50, 210.82, 10541, 1756.83],
      [30, 1084.03, 32520.9, 5420.15],
      [5, 803.19, 4015.95, 669.33],
      [4, 55.72, 222.88, 37.15],
      [10, 43.55, 435.5, 72.58],
      [2, 317.16, 634.32, 105.72],
      [4, 129.38, 517.52, 86.25],
      [1, 3620, 3620, 603.33],
    ];
    const r = validateUpdTotals(
      {
        totalSum: 57810.07,
        vatSum: 9634.99,
        items: rows.map(([qty, price, sum, vatSum]) => ({
          qty,
          price,
          sum,
          vatRate: 20,
    ...
    const flagged = (r.warnings ?? []).filter((w) => w.name === 'price_includes_vat');
    // Все девять, включая последнюю с qty = 1. Единичное количество НЕ делает
    // законной цену, равную сумме С НАЛОГОМ: там 3620 при стоимости без налога
    // 3016.67. Законен другой случай — когда цена совпадает со стоимостью БЕЗ
    // налога (см. тест выше про транспортные услуги), и его признак не трогает.
    expect(flagged).toHaveLength(9);
    expect(flagged.map((w) => (w.scope as { row: number }).row)).toEqual([
      1, 2, 3, 4, 5, 6, 7, 8, 9,
    ]);
  });
});
