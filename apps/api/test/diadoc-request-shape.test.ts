...
describe('способ передачи реквизитов приложения', () => {
  it('штатный обмен идёт единственным способом — в теле, без Authorization', async () => {
    // Диадок требует реквизиты в теле и отсутствие заголовка Authorization.
    // Повтор через Basic здесь был и убран: документация такого варианта не
    // допускает, а на боевой учётной записи он получил тот же отказ.
    ...
    await expect(auth.header()).rejects.toThrow(/invalid_client/);
