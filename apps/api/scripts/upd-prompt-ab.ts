...
async function main(): Promise<void> {
  ...
  // Условие активации: НОЛЬ по каждому из трёх счётчиков. Раньше выход
  // блокировали только пустой прогон и регрессии, а неразобравшиеся файлы и
  // ненайденные грузополучатели печатались справочно — и отчёт всё равно
  // заканчивался «можно активировать». Ровно эти два случая опаснее всего:
  // упавший разбор мог упасть ИЗ-ЗА нового промпта, а отсутствующий
  // грузополучатель означает, что добавляемое поле не работает — то есть
  // менять промпт незачем.
  ...
  if (checked === 0) {
    ...
    blockers.push('нет разобранных файлов');
  }
  if (failures.length > 0) {
    blockers.push(`не разобрались файлы: ${failures.length}`);
  }
  if (regressions.length > 0) {
    ...
    blockers.push(`регрессии: ${regressions.length}`);
  }
  if (consigneeMissing.length > 0) {
    blockers.push(`грузополучатель не найден: ${consigneeMissing.length}`);
  }

  if (blockers.length > 0) {
    console.log(`\nАКТИВИРОВАТЬ v9 НЕЛЬЗЯ — ${blockers.join('; ')}.`);
    console.log('Сначала разберитесь с перечисленным выше.');
    process.exitCode = 1;
  } else {
    console.log('\nРегрессий нет: все поля, стабильные у базы, совпали с новой версией.');
    console.log(`Грузополучатель найден везде, где обещает манифест (${consigneeFound.length}).`);
    console.log(
      'Осталось глазами сверить имя и ИНН грузополучателя на нескольких документах —' +
        ' и можно активировать промпт в Администрирование → Промпты.',
    );
  }
}
