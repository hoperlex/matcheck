...
describe('карточка документа — что значит расхождение сумм', () => {
  // Текст называет размер разрыва и что проверить, но НЕ объявляет причину
  // единственной: недостача бывает и от пропущенной строки, и от неверно
  // прочитанного итога, и от ошибки в числах одной позиции.
  it('сумма позиций меньше итога: назван разрыв и что проверить', async () => {
    window.localStorage.setItem(VALIDATION_LS_KEY, 'expanded');
    ...
    expect(
      screen.getByText(
        /сумма строк меньше итога на 1 043 585,00 ₽: проверьте пропуск строки или ошибку в числах/,
      ),
    ).toBeTruthy();
  });

  it('сумма позиций больше итога: назван разрыв и что проверить', async () => {
    window.localStorage.setItem(VALIDATION_LS_KEY, 'expanded');
    ...
    expect(
      screen.getByText(
        /сумма строк больше итога на 500 000,00 ₽: проверьте задвоение строки или ошибку в числах/,
      ),
    ).toBeTruthy();
  });
});
