...
export function DeliveriesHistory({
  ...
  const renderStatusCell = (r: Row) => (
    <StatusIconsCell
      ...
      extra={
        <>
          {/* Разбор документа нашёл расхождение или подозрение. Признак берётся
              с сервера (docAttention) — тот же, что у фильтра «Требует
              проверки» и у колонки выгрузки. Считать его на клиенте по
              sourceDocuments нельзя: у приёмки сигнал бывает только в сверке
              фото, а её сводка в этот список не приезжает — строка попадала в
              фильтр, но выглядела чистой. */}
          {r.docAttention === true && (
  ...
  // Легенда значков статуса: собираем реальные статусы из данных (code/label/
  // color), чтобы расшифровка всегда соответствовала столбцу «Статус».
