...
export const UpdPdfItemSchema = z.preprocess(
  ...
  z.object({
    ...
    // price — цена единицы С НДС (= sum / qty), а не «цена без налога»
    // из колонки 4 формы УПД. См. промпт v6 (миграция 0060).
    price: z.number().nullable().optional(),
    // sum — стоимость С НАЛОГОМ — всего по строке (колонка 9 формы УПД).
    // НЕ «Стоимость без налога» (колонка 5). Веб-портал отображает это
    // значение в колонке «Сумма» позиций УПД.
