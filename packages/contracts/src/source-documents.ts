...
export const UpdWarningNameSchema = z.enum([
  ...
  'unit_price_one',
  // Количество, умноженное на цену, даёт стоимость С НАЛОГОМ вместо стоимости
  // без него: модель прочитала колонку правее графы 4, и строка завышена ровно
  // на ставку. Арифметика документа при этом сходится сама с собой — итог
  // собран из тех же завышенных строк, поэтому денежные проверки молчат.
  'price_includes_vat',
  // В количестве стоит код единицы измерения из графы 2 бланка: «шт» с 796,
  // «упак» с 778, «м2» с 055. Настоящее количество при этом уезжает в цену,
  // а арифметика остаётся согласованной, поэтому денежные проверки молчат.
  'unit_code_as_qty',
  // Цена упаковки при штучном количестве: модель перебросила множитель между
  // количеством и ценой. На приёмке 13094 «2000 шт × 1,00 ₽» приехало как
  // «2 шт × 1000,00 ₽» — произведение то же, итог документа сходится, и все
  // шесть проверок молчат. Ловится только по виду чисел: цена ровно из одной
  // значащей цифры (так выглядит перенос множителя 10^k), штучная единица и
  // объявленная в наименовании фасовка, которой количество меньше.
  'price_is_pack_price',
  ...
  // Реквизиты совпали с уже загруженным документом, но СОДЕРЖИМОЕ этого не
  // подтверждает: разошлись итоги, число позиций или их состав. Дубликатом
  // такой документ не помечается и с планшета не убирается — совпадение
  // «поставщик + номер + дата» бывает и у разных отгрузок, а цена ошибки
  // несимметрична: скрытый документ означает недостачу материалов в приёмке.
  'duplicate_unconfirmed',
  // ── Пакетные предупреждения ────────────────────────────────────────────────
  // Три следующих имени относятся не к числам документа, а к его пакету: они
  // говорят, что при нарезке многостраничного файла что-то могло потеряться.
  // Поэтому они УСТОЙЧИВЫЕ: арифметику валидатор пересчитывает при каждом
  // сохранении, а эти вычислить заново нечем — их переносят из прошлого
  // снимка (см. mergePersistentUpdWarnings).
  //
  // Ни одно из них не меняет статус документа и не мешает приёмке: документ с
  // материалами обязан доехать до планшета, даже если к пакету есть вопросы.
  //
  // Страницы, уверенно опознанные как накладная или сертификат, в сегменты
  // УПД не идут — и до сих пор исчезали без всякого следа. Предупреждение
  // говорит: в файле были страницы, которые никто не разобрал.
  'dropped_pages_not_parsed',
  // Номера документов пакета образуют ряд с дыркой (4304…4309 без 4308).
  // Эвристика: непоследовательная нумерация законна, поэтому это повод
  // посмотреть файл, а не признак ошибки.
  'sibling_number_gap',
  // На страницах файла виден номер документа, которому не нашлось ни одного
  // распознанного документа в комплекте. В отличие от предыдущего — прямой
  // признак потери, а не догадка по ряду номеров.
  'page_doc_number_unaccounted',
  // В одном пакете два документа с одним номером, но разными поставщиком или
  // датой. Склейка экземпляров их не трогает — и правильно: один номер у
  // разных поставщиков встречается постоянно. Но так же выглядит и разрез не
  // по границе документа, поэтому пакет стоит посмотреть глазами.
  'sibling_number_duplicate',
]);
export type UpdWarningName = z.infer<typeof UpdWarningNameSchema>;

/**
 * Имена, которые валидатор вычислить не может: они появляются при сборке
 * пакета и обязаны пережить пересчёт validation (повторный разбор, склейку
 * сегментов, ручную правку документа).
 */
export const PERSISTENT_UPD_WARNING_NAMES = [
  'dropped_pages_not_parsed',
  'sibling_number_gap',
  'page_doc_number_unaccounted',
  'sibling_number_duplicate',
] as const satisfies readonly UpdWarningName[];

export function isPersistentUpdWarning(name: UpdWarningName): boolean {
  return (PERSISTENT_UPD_WARNING_NAMES as readonly UpdWarningName[]).includes(name);
}

export const UpdWarningSchema = z.object({
  name: UpdWarningNameSchema,
  scope: UpdCheckScopeSchema,
  // Подробности пакетных предупреждений: какие страницы выброшены, какого они
  // вида, какого номера не хватает. У арифметических подозрений деталей нет —
  // поле опционально и у прежних записей отсутствует.
  details: z
    .object({
      pages: z.array(z.number().int().positive()).optional(),
      pageKinds: z.array(z.string()).optional(),
      docNumbers: z.array(z.string()).optional(),
    })
    .optional(),
});
...
/**
 * Цена в строке указана С НДС, хотя графа 4 бланка — без налога.
 *
 * Признак арифметический и потому однозначный: количество, умноженное на цену,
 * даёт стоимость С НАЛОГОМ (графа 9) вместо стоимости без него (графа 5).
 * Модель прочитала колонку правее нужной, и вся строка завышена ровно на
 * ставку. За 30 дней таких строк 155 в 36 документах; пример — УПД № 223379,
 * где расхождение ровно 20 % во всех девяти строках.
 *
 * Почему это не ловится обычной сверкой сумм: числа самосогласованы. Итог
 * документа собирается из тех же завышенных строк, поэтому «Всего к оплате»
 * сходится сам с собой, и денежные проверки молчат.
 *
 * Три оговорки, без которых признак давал бы ложные срабатывания:
 *   * налог должен быть выделен ВЕРНО (vatSum отвечает ставке от базы). Иначе
 *     это другой дефект — модель посчитала НДС процентом от суммы с налогом, —
 *     и лечится он не тем же способом;
 *   * если произведение сходится и с базой тоже (мелкие суммы, где допуск
 *     перекрывает разницу), признак не предъявляется: доказательства нет;
 *   * нулевая ставка исключена: там графы 5 и 9 совпадают законно.
 */
export function suspectPriceIncludesVat(item: {
  qty?: number | null;
  price?: number | null;
  sum?: number | null;
  vatSum?: number | null;
  ...
  const { qty, price, sum, vatSum, vatRate } = item;
  if (qty == null || price == null || sum == null || vatSum == null) return false;
  if (![qty, price, sum, vatSum].every((v) => Number.isFinite(v))) return false;
  if (qty <= 0 || price <= 0 || sum <= 0 || vatSum <= 0) return false;

  const base = sum - vatSum;
  if (base <= 0) return false;

  // Налог выделен верно — значит документ обычный, а перепутана колонка цены.
  if (vatRate != null && Number.isFinite(vatRate) && vatRate > 0) {
    const actualRate = (vatSum / base) * 100;
    if (Math.abs(actualRate - vatRate) > 0.6) return false;
  }

  const product = qty * price;
  const tolerance = Math.max(1, sum * 0.005);
  const matchesWithVat = Math.abs(product - sum) <= tolerance;
  const matchesBase = Math.abs(product - base) <= Math.max(1, base * 0.005);

  // Сошлось с суммой С налогом и НЕ сошлось с базой — только тогда это улика.
  return matchesWithVat && !matchesBase;
}

/**
 * Код ОКЕИ по названию единицы измерения.
 *
 * В бланке УПД единица занимает ДВЕ графы: код (графа 2) и условное
 * обозначение (графа 2а). Модель регулярно принимает код за количество —
 * тогда «шт» приезжает в паре с qty = 796, «упак» с 778, «м2» с 055.
 * Настоящее количество при этом уезжает в цену.
 *
 * Написание единицы приходит как попало («шт» и «шт.», «м2» и «м²», «пог. м»),
 * поэтому сравнение идёт по нормализованной форме.
 */
const OKEI_BY_UNIT: Readonly<Record<string, number>> = {
  шт: 796,
  м: 6,
  м2: 55,
  м3: 113,
...
/**
 * В количестве стоит код единицы измерения, а не количество.
 *
 * Самый частый почерк «съехавших колонок» на бою: УПД-5 (ГРИФФ) — `qty = 796`
 * при единице «шт», настоящие 200 штук уехали в цену; № 2595 — `qty = 778` при
 * «упак», количество 20 стало ценой. Арифметика после такой подмены остаётся
 * внутренне согласованной, поэтому денежные проверки молчат.
 *
 * Код обязан совпасть с СОБСТВЕННОЙ единицей строки — иначе это просто число.
 * Совпадение вида «796 штук» теоретически возможно, но в паре с той же
 ...
 * множителя, поэтому `row_qty_price` сходится, итог документа собран из тех же
 * чисел, и `sum_total` тоже молчит. Существующие подозрения мимо: у
 * `qty_price_swap` требуется qty > price (2 < 1000), у `unit_price_one` — цена
 * ровно 1.
 *
 * Условия намеренно узкие, и каждое отсекает свой источник ложных срабатываний:
 *
 *  - цена из ОДНОЙ значащей цифры (1000, 5000, 400000, но не 1830 и не 526).
 *    Перенос множителя 10^k делает цену «слишком круглой»; настоящая цена так
 *    выглядит редко. Без этого условия признак давал 112 срабатываний за 90
 *    дней вместо шести;
 *  - штучная единица: у метров и килограммов дробное количество нормально;
 *  - фасовка объявлена в самом наименовании («200 шт/уп», «упак. 1000 шт.») и
 *    не меньше десяти — иначе под признак попадают обычные мелкие партии;
 *  - количество меньше фасовки: 6,15 упаковки мешков — это и есть след
 *    переноса, целые 10 упаковок подозрений не вызывают.
 *
 * Проверено на всей истории документов (11 734 строки): шесть срабатываний за
 * 90 дней, ложных среди них нет. Два настоящих дефекта — дюбель из приёмки
 * 13094 и «Мешки полипропиленовые (упак. 1000 шт.) — 6,15 шт × 5000 ₽».
 *
 * Предъявлять ТОЛЬКО там, где арифметика строки сошлась: если строка уже
 * красная, у проверяющего есть сигнал получше, а второй ярлык только запутает.
 */
export function suspectPackPriceScale(item: {
  qty: number | null;
  unit: string | null;
  price: number | null;
  nameRaw: string | null;
  ...
  if (qty == null || price == null) return false;
  if (!Number.isFinite(qty) || !Number.isFinite(price)) return false;
  if (qty <= 0 || price < 100) return false;

  // Цена состоит из одной значащей цифры: c × 10^k, где c — 1..9, k ≥ 2.
  if (!Number.isInteger(price)) return false;
  const magnitude = Math.pow(10, Math.floor(Math.log10(price)));
  if (magnitude < 100) return false;
  if (price % magnitude !== 0) return false;

  if (!PACK_UNITS.has(normalizeUnit(item.unit ?? ''))) return false;

  const pack = packSizeFromName(item.nameRaw);
  if (pack == null || pack < 10) return false;
  return qty < pack;
}

/** Единицы, при которых фасовка вообще имеет смысл. */
const PACK_UNITS = new Set(['шт', 'штук', 'штука']);

/**
 * Фасовка, объявленная в наименовании: «200 шт/уп», «(упак. 1000 шт.)», «100 шт.».
 *
 * Берётся первое число перед «шт» — именно так поставщики и пишут размер
 * упаковки. Если чисел несколько (размеры «10x300»), они стоят до слова и в
 * шаблон не попадают.
 */
function packSizeFromName(nameRaw: string | null): number | null {
  if (!nameRaw) return null;
  const m = /(\d+)\s*шт/i.exec(nameRaw);
  if (!m) return null;
  const n = Number(m[1]);
...
export const SourceDocumentListResponseSchema = z.object({
  ...
  /**
   * Сколько страниц получилось при нарезке списка.
   *
   * Страница режется по ПОСТАВКАМ — документы одной загрузки не разъезжаются
   * по разным страницам, — поэтому строк на ней около `limit`, а не ровно
   * столько. Вывести число страниц из `total / limit` из-за этого нельзя, и
   * пагинатор берёт его отсюда. `total` при этом остаётся числом документов:
   * надпись «Всего» на экране считает документы, а не машины.
   *
   * Необязательное: старые клиенты (планшет) поля не ждут и продолжают
   * считать страницы по-своему.
   */
...
export const OperationSourceDocumentSchema = z.object({
  ...
  /**
   * Сколько позиций в самом документе и сколько из них доехало до операции.
   *
   * Зачем. Приёмка 13157: в УПД три позиции, в приёмке две, и увидеть это было
   * негде — заголовок блока показывал «(2)», как будто столько и было. За
   * месяц таких приёмок 90–117, и ещё у 58–69 не связано ни одной позиции.
   * Проверено, что это не разделение документа между операциями: документ
   * привязан ровно к одной, а «лишние» позиции не использованы нигде.
   *
   * Оба поля опциональны: продюсер, который их не считает (черновик «из УПД» на
   * фронте, офлайн-снимок), просто не заполняет — заголовок тогда прежний.
   */
  itemsCount: z.number().int().nonnegative().optional(),
  /** Уникальные позиции документа, попавшие в операцию (COUNT DISTINCT). */
  coveredItemsCount: z.number().int().nonnegative().optional(),
});
...
export const UpdPdfItemSchema = z.preprocess(
  ...
  z.object({
    ...
    // Графы товарной накладной ТОРГ-12, по которым количество считается
    // кодом (domain/edo/torg12-qty). В бланке без цен количество товара
    // напрямую не напечатано: есть «Количество в одном месте» (графа 7) и
    // «Количество мест, штук» (графа 8), а в графе 10 «Количество (масса
    // нетто)» стоит масса в килограммах — её модель и отдаёт как qty.
    //
    // Умножение делает код, а не промпт: арифметика в промпте уже проверена
    // и ухудшает чтение колонок (v15/v16 против v13). Поля возвращает только
    // новая версия УПД-промпта; на прежних приходит undefined, и правило
    // молчит.
    qtyPerPlace: lenientNumber(),
    places: lenientNumber(),
    massNetKg: lenientNumber(),
