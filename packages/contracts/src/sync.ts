...
export const SseEventSchema = z.object({
  ...
  /**
   * Объект, к которому относится событие. Маршрут /events отдаёт инспектору
   * только события его объекта — без этого изменение на любом объекте будило
   * планшеты всех объектов сразу (712 запросов /sync за полчаса при медиане
   * обработчика 3,3 с, замер 25.08).
   *
   * Опционально по трём причинам сразу. У справочников (материалы,
   * контрагенты) объекта нет вовсе. Мобильный парсер игнорирует незнакомые
   * ключи, поэтому клиент менять не нужно. И главное: событие БЕЗ siteId
   * доставляется всем, поэтому разметка источников может ехать частями —
   * неразмеченный источник даёт лишний трафик, а не потерянное событие.
   */
