...
export const PublicRejectReasonSchema = z.enum([
  ...
  'heic_unsupported',
  // Единственная причина, которая появляется ПОСЛЕ входного фильтра: файл
  // прошёл проверки, но не лёг в хранилище. Для поставщика это тот же класс
  // «пришлите ещё раз», поэтому код живёт в общем списке, а не отдельным полем.
  // Остальные файлы отправки при этом приняты, и повторная отправка дозагрузит
  // только пропущенное.
  'storage_failed',
]);
